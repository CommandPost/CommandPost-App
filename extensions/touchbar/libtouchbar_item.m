...
/// === hs.touchbar.item ===
///
/// This module is used to create and manipulate touchbar item objects which can added to `hs.touchbar.bar` objects and displayed in the Touch Bar of new Macintosh Pro laptops or with the virtual Touch Bar provided by `hs.touchbar.virtual`.
///
...
static const char * const USERDATA_TAG = "hs.touchbar.item" ;
static const char * const BAR_UD_TAG   = "hs.touchbar.bar" ;
...
static NSDictionary *itemTypeStrings ; // assigned in luaopen_hs_libtouchbar_item
...
/// hs.touchbar.item.newGroup([identifier]) -> touchbarItemObject
/// Constructor
...
/// hs.touchbar.item.newSlider([identifier]) -> touchbarItemObject
/// Constructor
...
/// hs.touchbar.item.newCanvas(canvas, [identifier]) -> touchbarItemObject
/// Constructor
...
///
/// Notes:
///  * The touch bar object will be proportionally resized so that it has the same height as the touchbar if it does not already. Currently this is 30, but in case future models differ from this, you can get the current value with from `hs.touchbar.size`.
...
/// hs.touchbar.item.newButton([title], [image], [identifier]) -> touchbarItemObject
/// Constructor
...
///
/// Notes:
///  * You can change the button's title with [hs.touchbar.item:title](#title) only if you initially assign one with this constructor.
///  * You can change the button's image with [hs.touchbar.item:image](#title) only if you initially assign one with this constructor.
...
/// hs.touchbar.item:visibilityCallback([fn]) -> touchbarItemObject | current value
/// Method
...
///
///  * See also the notes for [hs.touchbar.item:isVisible](#isVisible) and `hs.touchbar.bar.visibilityCallback`.
...
/// hs.touchbar.item:sliderMinImage([image]) -> touchbarItemObject | current value
/// Method
...
///
/// Notes:
///  * This method will generate an error if the touchbar item was not created with the [hs.touchbar.item.newSlider](#newSlider) constructor.
...
/// hs.touchbar.item:sliderMaxImage([image]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:sliderMin([value]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:sliderMax([value]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:sliderValue([value]) -> touchbarItemObject | current value
/// Method
/// Get or set the current value for a slider touchbar item.
///
/// Parameters:
///  * `value` - an optional number specifying the value to set for the slider. This value will be automatically constrained to the current minimum and maximum as set by [hs.touchbar.item:sliderMin](#sliderMin) and [hs.touchbar.item:sliderMax](#sliderMax).
///
/// Returns:
///  * if an argument is provided, returns the touchbarItem object; otherwise returns the current value
///
...
/// hs.touchbar.item:groupTouchbar([touchbar]) -> touchbarItemObject | current value
/// Method
/// Get or set the bar object which contains the touchbar items that belong to the group touchbar item.
///
/// Parameters:
///  * `touchbar` - an optional `hs.touchbar.bar` object containing the touchbar items to display when this group touchbar item is present in the touchbar.
///
/// Returns:
///  * if an argument is provided, returns the touchbarItem object; otherwise returns the current value
///
/// Notes:
///  * This method will generate an error if the touchbar item was not created with the [hs.touchbar.item.newGroup](#newGroup) constructor.
...
///  * See also [hs.touchbar.item:groupItems](#groupItems)
...
/// hs.touchbar.item:customizationLabel([label]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:buttonImage([image]) -> touchbarItemObject | hs.image object
/// Method
...
///
/// Notes:
///  * This method will generate an error if the touchbar item was not created with the [hs.touchbar.item.newButton](#newButton) constructor.
...
/// hs.touchbar.item:buttonTitle([title]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:canvasWidth([width]) -> touchbarItemObject | current value
/// Method
...
///
/// Notes:
///  * This method will generate an error if the touchbar item was not created with the [hs.touchbar.item.newCanvas](#newCanvas) constructor.
...
/// hs.touchbar.item:canvasClickColor([color]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:enabled([state]) -> touchbarItemObject | current value
/// Method
...
///
/// Notes:
///  * This method will generate an error if the touchbar item was not created with the [hs.touchbar.item.newButton](#newButton) or [hs.touchbar.item.newCanvas](#newCanvas) constructors.
    ...
    switch(obj.itemType) {
        ...
        case TBIT_canvas: {
            if (lua_gettop(L) == 1) {
                ...
                ((NSButton *)obj.view).enabled = (int)lua_toboolean(L, 2) ;
                lua_pushvalue(L, 1) ;
            }
        } break ;
        default:
...
/// hs.touchbar.item:buttonSize([size]) -> touchbarItemObject | current value
/// Method
...
/// hs.touchbar.item:identifier() -> string
/// Method
...
/// hs.touchbar.item:itemType() -> string
/// Method
...
/// hs.touchbar.item:isVisible() -> boolean
/// Method
...
/// hs.touchbar.item:visibilityPriority([priority]) -> touchbarItemObject | current value
/// Method
...
///  * Some predefined visibility values are defined in [hs.touchbar.item.visibilityPriorities](#visibilityPriorities), though others are allowed. The default priority for an item object is `hs.touchbar.item.visibilityPriorities.normal`.
...
/// hs.touchbar.item:callback([fn]) -> touchbarItemObject | current value
/// Method
...
///    * Items constructed with [hs.touchbar.item.newButton](#newButton):
///      * the touchbar item itself
///
///    * Items constructed with [hs.touchbar.item.newCanvas](#newCanvas):
///      * the touchbar item itself
...
///
///    * Items constructed with [hs.touchbar.item.newGroup](#newGroup):
...
///
///    * Items constructed with [hs.touchbar.item.newSlider](#newSlider):
///      * the touchbar item itself
///      * a number or string as follows:
///        * if the image assigned with [hs.touchbar.item:sliderMinImage](#sliderMinImage) is touched, the string "minimum".
///        * if the image assigned with [hs.touchbar.item:sliderMaxImage](#sliderMaxImage) is touched, the string "maximum".
///        * if the slider knob is moved to a new position, returns a number between [hs.touchbar.item:sliderMin](#sliderMin) and [hs.touchbar.item:sliderMax](#sliderMax) indicating the new position.
...
/// hs.touchbar.item:addToSystemTray(state) -> touchbarItemObject
/// Method
...
///  * Adding a new item will hide any previous item assigned; however they do appear to stack, so removing an existing item with this method, or if it has bar object attached with [hs.touchbar.item:presentModalBar](#presentModalBar) and you dismiss the bar with `hs.touchbar.bar:dismissModalBar`, the previous item should become visible again.
///
///  * At present, there is no known way to determine which item is currently displayed in the System Tray or detect when a specific item is replaced ([hs.touchbar.item:isVisible](#isVisible) returns false). Please submit an issue if you know of a solution.
///
...
/// hs.touchbar.item.visibilityPriorities[]
/// Constant
/// Predefined visibility priorities for use with [hs.touchbar.item:visibilityPriority](#visibilityPriority)
///
/// A table containing key-value pairs of predefined visibility priorities used when the touch bar isn't large enough to display all of the items which are eligible for presentation. Items with lower priorities are hidden first. These numbers are only suggestions and other numbers are also valid for use with [hs.touchbar.item:visibilityPriority](#visibilityPriority).
///
...
int luaopen_hs_libtouchbar_item(lua_State* L) {
    LuaSkin *skin = [LuaSkin sharedWithState:L] ;
