--- === hs.touchbar ===
---
--- This module and its submodules provide support for manipulating the Apple Touch Bar on newer Macbook Pro laptops. For machines that do not have a touchbar, the `hs.touchbar.virtual` submodule provides a method for mimicing one on screen.
---
...
---
--- This module and it's submodules require a mac that is running macOS 10.12.1 build 16B2657 or newer. If you wish to use this module in an environment where the end-user's machine may not have a new enough macOS release version, you should always check the value of [hs.touchbar.supported](#supported) before trying to create the Touch Bar and provide your own fallback or message. By supplying the argument `true` to this function, the user will be prompted to upgrade if necessary.
---
...
local USERDATA_TAG = "hs.touchbar"
local module       = require("hs.libtouchbar")
...
module.virtual   = require("hs.libtouchbar_virtual")
module.item      = require("hs.libtouchbar_item")
module.bar       = require("hs.libtouchbar_bar")
-- module._debug    = require("hs.libtouchbar_debug")
...
--- hs.touchbar.virtual:toggle([duration]) -> touchbarObject
--- Method
...
--- hs.touchbar.virtual:atMousePosition() -> touchbarObject
--- Method
...
--- hs.touchbar.virtual:centered([top]) -> touchbarObject
--- Method
...
--- hs.touchbar.item:presentModalBar(touchbar, [dismissButton]) -> touchbarItemObject
--- Method
...
---
--- Parameters:
---  * `touchbar` - An `hs.touchbar.bar` object of the bar to display modally in the touch bar display.
...
---
--- Notes:
---  * If you specify `dismissButton` as false, then you must use `hs.touchbar.bar:minimizeModalBar` or `hs.touchbar.bar:dismissModalBar` to remove the modal bar from the touch bar display.
---    * Use `hs.touchbar.bar:minimizeModalBar` if you want the item to reappear in the System Tray (if it was present before displaying the bar).
---
...
---
---  * This method is actually a wrapper to `hs.touchbar.bar:presentModalBar` provided for convenience.
---
...
--- hs.touchbar.item:groupItems([itemsTable]) -> touchbarItemObject | current value
--- Method
...
---  * This is a convenience method which creates an `hs.touchbar.bar` object and uses [hs.touchbar.item:groupTouchbar](#groupTouchbar) to assign the items to the group item.
