...
/// === hs.touchbar.bar ===
///
/// This module is used to create and manipulate bar objects which can be displayed in the Touch Bar of new Macintosh Pro laptops or with the virtual Touch Bar provided by `hs.touchbar.virtual`.
///
...
static const char * const USERDATA_TAG = "hs.touchbar.bar" ;
static const char * const ITEM_UD_TAG  = "hs.touchbar.item" ;
...
// https://github.com/asmagill/hs.touchbar/issues/1 and
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if (context == myKVOContext && [keyPath isEqualToString:@"visible"]) {
        if (self.visibilityCallbackRef != LUA_NOREF) {
            LuaSkin *skin = [LuaSkin sharedWithState:NULL] ;
            lua_State *L  = [skin L] ;
            // KVO seems to be slow and may not invoke the callback until after gc during a reload
            if ([skin pushLuaRef:refTable ref:self.visibilityCallbackRef] != LUA_TNIL) {
...
/// hs.touchbar.bar.toggleCustomization() -> None
/// Function
...
/// hs.touchbar.bar:isVisible() -> boolean
/// Method
...
///
/// Notes:
///  * The value returned by this method changes as expected when the [hs.touchbar.bar:dismissModalBar](#dismissModalBar) or [hs.touchbar.bar:minimizeModalBar](#minimizeModalBar) methods are used.
///  * It does *NOT* reliably change when when the system dismiss button is used (when the second argument to [hs.touchbar.bar:presentModalBar](#presentModalBar) or `hs.touchbar.item:presentModalBar` is true (or not present)).  This is being investigated but at present no workaround is known.
...
/// hs.touchbar.bar:itemIdentifiers() -> table
/// Method
...
///
/// Notes:
///  * If the user has not customized the bar, the list of identifiers will match the list provided by [hs.touchbar.bar:defaultIdentifiers()](#defaultIdentifiers).
...
/// hs.touchbar.bar:customizationLabel([label]) -> barObject | string
/// Method
...
/// hs.touchbar.bar:principleItem([identifier]) -> barObject | string
/// Method
...
///
///  * the identifier specified must belong to a touchbar items already assigned to the bar object with [hs.touchbar.bar:templateItems](#templateItems) or one of the built in identifier values defined in [hs.touchbar.bar.builtInIdentifiers](#builtInIdentifiers).
...
/// hs.touchbar.bar:escapeKeyReplacement([identifier]) -> barObject | string
/// Method
...
///
///  * the identifier specified must belong to a touchbar items already assigned to the bar object with [hs.touchbar.bar:templateItems](#templateItems).
...
/// hs.touchbar.bar:customizableIdentifiers([identifiersTable]) -> barObject | table
/// Method
...
///
/// Notes:
///  * the identifiers specified must belong to touchbar items already assigned to the bar object with [hs.touchbar.bar:templateItems](#templateItems) or one of the built in identifier values defined in [hs.touchbar.bar.builtInIdentifiers](#builtInIdentifiers).
...
/// hs.touchbar.bar:requiredIdentifiers([identifiersTable]) -> barObject | table
/// Method
...
/// hs.touchbar.bar:defaultIdentifiers([identifiersTable]) -> barObject | table
/// Method
...
/// hs.touchbar.bar:templateItems([itemsTable]) -> barObject | table
/// Method
/// Get or set an array of `hs.touchbar.item` objects that can be presented by the bar object.
///
/// Parameters:
///  * `itemsTable` - an optional table containing `hs.touchbar.item` objects that can be presented by the bar object.
///
/// Returns:
///  * If an argument is provided, returns the barObject; otherwise returns the current value.
///
...
/// hs.touchbar.bar:itemForIdentifier([identifier]) -> touchbarItemObject | nil
/// Method
/// Returns the touchbarItemObject for the identifier specified.
///
/// Parameters:
///  * `identifier` - a string specifying the touchbarItem object to get from the items assigned to the bar with [hs.touchbar.bar:templateItems](#templateItems).
///
/// Returns:
...
/// hs.touchbar.bar:visibilityCallback([fn]) -> barObject | fn
/// Method
...
///
///  * This callback is invoked when the [hs.touchbar.bar:dismissModalBar](#dismissModalBar) or [hs.touchbar.bar:minimizeModalBar](#minimizeModalBar) methods are used.
///  * This callback is *NOT* invoked when the system dismiss button is used (when the second argument to [hs.touchbar.bar:presentModalBar](#presentModalBar) or `hs.touchbar.item:presentModalBar` is true (or not present)). This is being investigated but at present no workaround is known.
...
/// hs.touchbar.bar:presentModalBar([itemObject], [dismissButton]) -> barObject
/// Method
/// Presents the bar in the touch bar display modally.
///
/// Parameters:
///  * `itemObject`    - an optional `hs.touchbar.item` object which, if currently attached to the system tray, will be hidden while the bar is visible modally.
...
///
/// Notes:
///  * If you specify `dismissButton` as false, then you must use [hs.touchbar.bar:minimizeModalBar](#minimizeModalBar) or [hs.touchbar.bar:dismissModalBar](#dismissModalBar) to remove the modal bar from the touch bar display.
///
...
///
///  * If you specify `itemObject` and the object is not currently attached to the system tray (see `hs.touchbar.item:addToSystemTray)`, or if you do not have "Touch Bar Shows" set to "App Controls With Control Strip" set in the Keyboard System Preferences, providing this argument has no effect.
///
...
static int touchbar_presentSystemModalFunctionBar(lua_State *L) {
    ...
    if (lua_type(L, -1) == LUA_TBOOLEAN) {
        closeBox = (int)lua_toboolean(L, -1) ;
        lua_pop(L, 1) ;
    }
    if (lua_gettop(L) > 1) {
...
/// hs.touchbar.bar:dismissModalBar() -> barObject
/// Method
...
///
/// Notes:
///  * If an `itemObject` was specified with [hs.touchbar.bar:presentModalBar](#presentModalBar) or if the bar was displayed with `hs.touchbar.item:presentModalBar`, this method will ***not*** restore the item to the system tray.
///
///  * This method uses undocumented functions and/or framework methods and is not guaranteed to work with future updates to macOS. It has currently been tested with 10.12.6.
...
/// hs.touchbar.bar:minimizeModalBar() -> barObject
/// Method
...
///
/// Notes:
///  * If an `itemObject` was specified with [hs.touchbar.bar:presentModalBar](#presentModalBar) or if the bar was displayed with `hs.touchbar.item:presentModalBar`, this method ***will*** restore the item to the system tray.
///
///  * This method is the same as pressing the `dismissButton` if it was not set to false when [hs.touchbar.bar:presentModalBar](#presentModalBar) or `hs.touchbar.item:presentModalBar` was invoked.
///
///  * This method uses undocumented functions and/or framework methods and is not guaranteed to work with future updates to macOS. It has currently been tested with 10.12.6.
...
/// hs.touchbar.bar.builtInIdentifiers[]
/// Constant
...
int luaopen_hs_libtouchbar_bar(lua_State* L) {
